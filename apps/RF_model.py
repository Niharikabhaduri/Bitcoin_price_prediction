...
RF_module = Blueprint('RF_model',__name__)

@RF_module.route('/Generate-actual-predicted-price-rf',methods=['GET'])
...
@RF_module.route('/Generate-feature-importance-rf', methods = ['GET'])
@cross_origin(supports_credentials=True)
def feature_importance():
    ...
    predictions_scaled = RF_model.predict(X_test).reshape(-1, 1)
...
@RF_module.route('/Generate-correlation-rf')
@cross_origin(supports_credentials=True)
def correlation():
    RF_model = pickle.load(open('RF_model.pkl','rb'))
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
...
@RF_module.route('/Generate-evaluation-metrics-rf')
@cross_origin(supports_credentials=True)
def evaluation():
    RF_model = pickle.load(open('RF_model.pkl','rb'))
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
    ...
    plt.ylabel('metrics')
    plt.title('Evaluation results of Random Forest')
    plt.legend()
