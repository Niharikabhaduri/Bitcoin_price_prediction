...
GRU_module = Blueprint('GRU_model',__name__)

@GRU_module.route('/Generate-actual-predicted-price-gru',methods=['GET'])
...
def Generate_ap_price():
    ...
    predictions_scaled = GRU_model.predict(X_test).reshape(-1, 1)
...
@GRU_module.route('/Generate-feature-importance-gru', methods = ['GET'])
def feature_importance():
    ...
    result = permutation_importance(GRU_model, X_test, y_test, n_repeats=10, random_state=42, scoring = "r2")
...
@GRU_module.route('/Generate-correlation-gru')
def correlation():
    GRU_model =  load_model('GRU_model.h5')
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
    scaler_Y = pickle.load(open('scaler_Y.pkl', 'rb'))
    ...
    predictions = GRU_model.predict(X_test).reshape(-1, 1)
    X_test = X_test.reshape(-1,3)
...
@GRU_module.route('/Generate-evaluation-metrics-gru')
def evaluation():
    GRU_model =  load_model('GRU_model.h5')
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
    scaler_Y = pickle.load(open('scaler_Y.pkl', 'rb'))
    ...
    plt.ylabel('metrics')
    plt.title('Evaluation results of Grated Recurrent Model')
    plt.legend()
