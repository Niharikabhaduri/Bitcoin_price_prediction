...
GB_module = Blueprint('GB_model',__name__)

@GB_module.route('/Generate-actual-predicted-price-gb',methods=['GET'])
...
def Generate_ap_price():
    ...
    predictions_scaled = GB_model.predict(X_test).reshape(-1, 1)
...
@GB_module.route('/Generate-feature-importance-gb', methods = ['GET'])
@cross_origin(supports_credentials=True)
...
@GB_module.route('/Generate-correlation-gb')
@cross_origin(supports_credentials=True)
def correlation():
    GB_model = pickle.load(open('GB_model.pkl','rb'))
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
    ...
    predictions = GB_model.predict(X_test).reshape(-1, 1)
...
@GB_module.route('/Generate-evaluation-metrics-gb')
@cross_origin(supports_credentials=True)
def evaluation():
    GB_model = pickle.load(open('GB_model.pkl','rb'))
    scaler_X = pickle.load(open('scaler_X.pkl', 'rb'))
    ...
    plt.ylabel('metrics')
    plt.title('Evaluation results of Gradient Boosting')
    plt.legend()
